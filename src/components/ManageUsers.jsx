...
const ManageUsers = () => {
                ...
                if (res.data.success) {
                    if (category === 'admin') {
                        ...
                        const studentsList = usersList.map(user => [
                            <input type="checkbox" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" onClick={(event) => selectHandler(event, user)} />,
                            `${user.firstName} ${user.lastName}`, user.email, data.map.sections.find(section => section.value === user.section).name, user.lrn])
    ...
    const deleteUserHandler = (usersDocs) => {
        ...
        const usersToBeDeletedStrings = usersToBeDeleted.map(user => `${user.firstName} ${user.lastName}`);
        setPopupModal({
            message: `Are you sure you want to delete these users: ${usersToBeDeletedStrings.join(', ')}?`, variant: 'danger', handler: () => {
...
console.log("userDoc", userDoc);
                    ...
                    params.usersDocs = JSON.stringify(usersDocs.map(userDoc => {
                ...
                axios.delete('/api/v1/users/users', {
                    params
                })
                    .then(res => {
                        if (res.data.success) {
                            setToast({ message: res.data.msg, icon: 'check', lifetime: 10000 })
                            if (category === 'admin') {
                                setAdminUsers(prevState => {
                                    const updatedUsersList = prevState.filter(user => ids.find(id => id !== user._id));
                                    return updatedUsersList;
                                });
                            } else if (category === 'parent') {
                                setParentUsers();
                            } else if (category === 'student') {
                                const LRNs = ids.map(id => studentUsers.find(user => user._id === id).lrn)
                                const studentsRowsCopy = studentsRows;
                                let updatedUsersList = studentUsers.filter(user => !LRNs.includes(user.lrn));
                                for (const LRN of LRNs) {
                                    studentsRowsCopy.splice(studentsRowsCopy.indexOf(studentsRowsCopy.find(row => LRNs.includes(parseInt(row[3])))), 1);
                                }
            ...
            {isLocalDataReady ?
                <div>
                    ...
                    <div className={`flex justify-between items-center`}>
                        <ButtonGroup options={categories} stateHandler={setCategory} state={category} />
                        <Button className={`${category !== 'admin' ? 'block' : 'hidden'}`} variant="danger" size="small" type="button" onClick={() => { deleteUserHandler(usersToBeDeletedDocs) }} disabled={usersToBeDeletedLength === 0} shouldRender={shouldRender}>Delete Selected Users</Button>
                    </div>
                    <div className="py-3"></div>
                    {category === 'admin' ? isAdminUsersReady ?
                        <>
                            <ul className="flex flex-row flex-wrap max-w-full">
                                {adminUsers.map((user, i) => (
                                    <li key={i} className="w-[21rem] mr-5 mb-5"><UserCard account={user} map={data.map} action='remove' primaryHandler={() => { deleteUserHandler([user]) }} /></li>
                                ))}
                            </ul>
                        </>
                        :
                    ...
                    {category === 'parent' ? isParentUsersReady ?
                        <>
                            <p>parent</p>
                        </>
                        :
                    ...
                    {category === 'student' ? isStudentUsersReady ?
                        <>
                            {studentsRows.length === 0 ? <p>No student users found.</p> : <Table headersList={['Select', 'Name', 'Email', 'Section', 'LRN']} itemsList={studentsRows} />}
                        </>
                        :
                        <ContentFallbackCards1 />
                        : ''
